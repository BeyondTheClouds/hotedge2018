...
In a bottom-up design, the system is not limited by what is
available through VIM APIs. Specifically, by making OpenStack natively
collaborative, several `local' features can be supported across the entire
...
However, the efficiency and ease of use brought by native collaboration come 
at a cost. Since OpenStack has not been designed to be collaborative, most
...
and none individually seems to meet all requirements of \edge scenarios.
...
existing VIM codebase while the bottom-up approach, despite the impact 
